...
export const Route=createFileRoute('/api/assistant/analyze')({server:{handlers:{POST:async({request})=>{
 ...
 void(async()=>{try{const chat=await import('@/lib/assistant-chat.server');send({result:await chat.extract(owner,parsed.data.id,parsed.data.image,parsed.data.mode)});}catch(e){send({busy:e instanceof Error&&e.name==='AssistantBusyError',error:e instanceof Error?e.message:'Falha na leitura'});}finally{parsed.data.image.data='';clearInterval(heartbeat);if(!closed){closed=true;controller.close();}}})();},cancel(){/* operação em andamento conclui sem repetir escritas */}});
