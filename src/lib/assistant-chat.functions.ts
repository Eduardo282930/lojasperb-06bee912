...
const input=z.object({id:z.string().uuid().optional(),action:z.enum(['state','register','text','price','sync','cancel','clear','instructions']),productId:z.string().uuid().optional(),text:z.string().max(12000).optional(),price:z.number().positive().max(1000000).optional()});
